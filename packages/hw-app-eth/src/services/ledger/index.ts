...
import { Interface } from "@exodus/ethersproject-abi";
import {
  signDomainResolution,
  signAddressResolution,
} from "@exodus/ledgerhq-domain-service/signers/index.js";
...
import { decodeTxInfo, tokenSelectors, hexBuffer, nftSelectors, mergeResolutions } from "../../utils";
      ...
      if (erc20OfInterest && erc20OfInterest.length && abi) {
        ...
        for (const path of erc20OfInterest) {
          const erc20ContractAddress = path.split(".").reduce((value, seg) => {
            ...
            if (Object.prototype.hasOwnProperty.call(value, seg)) {
              return value[seg];
            }
...
const resolveTransaction: LedgerEthTransactionService["resolveTransaction"] = async (
  ...
  const rawTx = hexBuffer(rawTxHex);
