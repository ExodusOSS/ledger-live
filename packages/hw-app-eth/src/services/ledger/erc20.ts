...
import { signatures as signaturesByChainId } from "@exodus/ledgerhq-cryptoassets/lib/data/evm/index.js";
...
export const findERC20SignaturesInfo = async (
  ...
  const baseURL = new URL(cryptoassetsBaseURL);
  const sigsURL = url`${baseURL}/evm/${chainId}/erc20-signatures.json`;
  const blob = await axios
    .get<string>(sigsURL.toString())
    .then(({ data }) => {
      if (!data || typeof data !== "string") {
        throw new Error(`ERC20 signatures for chainId ${chainId} file is malformed ${sigsURL}`);
      }
      return data;
    })
    .catch(e => {
      log("error", "could not fetch from " + sigsURL + ": " + String(e));
      return null;
    });
...
const get: (chainId: number) => API | null = (() => {
  const cache: Record<number, API> = Object.create(null);
  return chainId => {
