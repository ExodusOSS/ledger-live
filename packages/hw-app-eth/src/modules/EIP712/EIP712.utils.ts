...
import EIP712CAL from "@exodus/ledgerhq-cryptoassets/lib/data/eip712";
...
export const makeTypeEntryStructBuffer = ({ name, type }: EIP712MessageTypesEntry): Buffer => {
  ...
  if (typeof typeSizeInBits === "number" && typeSizeInBits >= 0) {
...
export const getSchemaHashForMessage = (message: EIP712Message): string => {
  ...
  return createHash('sha224').update(JSON.stringify(sortedTypes).replace(" ", "")).digest().toString('hex')
};
