import { encode, decode } from "@exodus/ethersproject-rlp";
...
export function hexBuffer(str: string): Buffer {
  if (!/^(0x)?([a-zA-Z0-9]{2})*$/.test(str)) {
    throw new Error(`invalid hex input: ${str}`)
  }
 ...
 */
export function intAsHexBytes(int: number, bytes: number): string {
  if (int < 0) {
    throw new Error("can't encode negative number as hex");
  }
  if (Number.isNaN(int)) {
    throw new Error("can't encode NaN as hex");
  }
  const ret = int.toString(16).padStart(2 * bytes, "0");
  if (ret.length !== bytes * 2) {
    throw new Error(`invalid byte length ${bytes} ${int}`);
  }
  return ret;
}
