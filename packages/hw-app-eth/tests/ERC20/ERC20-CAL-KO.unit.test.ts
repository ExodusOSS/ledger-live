...
import evms from "@exodus/ledgerhq-cryptoassets/lib/data/evm/index";
...
jest.mock("@exodus/ledgerhq-cryptoassets/lib/data/evm/index", () => ({
  get signatures() {
    return {
      1: SignatureCALEth,
    };
  ...
  describe("ERC20 is not in local CAL", () => {
    it("shouldn't break if the dynamic CAL is malformed", async () => {
      ...
      const resolution = await ledgerService.resolveTransaction(
        txHex,
        { cryptoassetsBaseURL: "https://working" },
        resolutionConfig
      );
    ...
    it("should ask for blind sign if not in dynamic & local CAL", async () => {
      ...
      const resolution = await ledgerService.resolveTransaction(
        txHex,
        { cryptoassetsBaseURL: "https://notworking" },
        resolutionConfig
      );
