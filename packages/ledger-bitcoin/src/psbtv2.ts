...
import * as bjs from '@exodus/bitcoinjs';
...
export class PsbtV2 {
  ...
  getInputBip32DerivationPublicKeys(inputIndex: number): readonly Buffer[] {
    return this.getInputKeyDatas(inputIndex, psbtIn.BIP32_DERIVATION);
  }
  ...
  getInputTapBip32DerivationPublicKeys(inputIndex: number): readonly Buffer[] {
    return this.getInputKeyDatas(inputIndex, psbtIn.TAP_BIP32_DERIVATION);
  }
  getInputTapBip32Derivation(
    ...
    if (pubkey.length != 32)
      throw new Error('Invalid pubkey length: ' + pubkey.length);
    const buf = this.getInputOptional(inputIndex, psbtIn.TAP_BIP32_DERIVATION, pubkey);
    if (!buf) return undefined;
