...
export const getDeviceModel = (id: DeviceModelId): DeviceModel => {
  const info = Object.prototype.hasOwnProperty.call(devices, id) && devices[id];
...
const serviceUuidToInfos: Record<string, BluetoothInfos> = Object.create(null);

for (const deviceModel of devicesList) {
  const { bluetoothSpec } = deviceModel;
  if (bluetoothSpec) {
    for (const spec of bluetoothSpec) {
