...
//
// TLDR: /!\ Do not remove the /lib part in the import statements below (@exodus/ledgerhq-devices/lib) ! /!\
...
} from "@exodus/ledgerhq-devices";
import type { DeviceModel } from "@exodus/ledgerhq-devices";
...
import { firstValueFrom } from "./rxjsExtra";
 ...
 */
const transportsCache: { [deviceId: string]: BleTransport } = Object.create(null);
const bluetoothInfoCache: { [deviceUuid: string]: BluetoothInfos } = Object.create(null); // Allows us to give more granulary error messages.

// connectOptions is actually used by react-native-ble-plx even if comment above ConnectionOptions says it's not used
let connectOptions: Record<string, unknown> = Object.assign(Object.create(null), {
...
const getConnectOptionsWithTimeout = (timeoutMs: number | undefined) => {
  const _connectOptions = Object.assign(Object.create(null), connectOptions)
  _connectOptions.timeout = timeoutMs
...
async function open(
  ...
  { rxjsScheduler }: { rxjsScheduler?: SchedulerLike } = Object.create(null),
) {
  ...
  if (typeof deviceOrId === "string") {
    ...
    if (!device) {
      ...
      try {
        device = await bleManagerInstance().connectToDevice(deviceOrId, getConnectOptionsWithTimeout(timeoutMs));
      } catch (e: any) {
        ...
        if (e.errorCode === BleErrorCode.DeviceMTUChangeFailed) {
          ...
          connectOptions = Object.create(null);
  ...
  if (!(await device.isConnected())) {
    ...
    try {
      await device.connect(getConnectOptionsWithTimeout(timeoutMs));
    } catch (error: any) {
      ...
      if (error.errorCode === BleErrorCode.DeviceMTUChangeFailed) {
        ...
        const { deviceModel } = bluetoothInfoCache[device.id] || Object.create(null);
        const { productName } = deviceModel || Object.create(null);
...
export default class BleTransport extends Transport {
  ...
  constructor(
    ...
    notifyObservable: Observable<Buffer | Error>,
    deviceModel: DeviceModel,
    { context, rxjsScheduler }: { context?: TraceContext; rxjsScheduler?: SchedulerLike } = Object.create(null),
  ) {
  ...
  exchange = (
    message: Buffer,
    { abortTimeoutMs }: { abortTimeoutMs?: number } = Object.create(null),
  ): Promise<Buffer> => {
